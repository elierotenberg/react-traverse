...
export default function traverse(node, visitor) {
  ...
  const path = {
    ...
    traverse(childNode, childVisistor = visitor) {
      return traverse(childNode, childVisistor);
    },
    traverseChildren() {
      return React.Children.toArray(path.node.props.children).map(path.traverse);
    },
    visitor,
  };
  if(node === null || node === void 0 || typeof node === 'boolean') {
