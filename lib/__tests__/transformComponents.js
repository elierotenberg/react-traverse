...
describe('transformComponents', () => {
  describe('replaces divs with spans', () => {
    ...
    function replaceDivsWithSpansInComponent(type) {
      return wrapRender(replaceDivsWithSpansInNode)(type);
    }
    ...
    it('replaces in stateless components', () => {
      ...
      const TransformedFoo = transformComponents(replaceDivsWithSpansInComponent)(Foo);
    ...
    it('replaces in nested stateless components', () => {
      ...
      const TransformedFoo = transformComponents(replaceDivsWithSpansInComponent)(Foo);
      const actual = <TransformedFoo />;
      const expected = <span className='foo'>
        <span className='bar'>
          {'bar'}
        </span>
      </span>;
  ...
  describe('hashes all classNames', () => {
    ...
    function hashAllClassNamesInNode(hashKey) {
      return (node) => traverse(node, {
        DOMElement(path) {
          ...
          const nextProps = Object.assign({}, path.node.props, { children: void 0 });
    ...
    function createTransformComponent(hashKey) {
      return (type) => wrapRender(hashAllClassNamesInNode(hashKey))(type);
    }
    ...
    it('hashes all classNames correctly', () => {
      ...
      const actual = <Foo />;
      const expected = <div className={hashClassName(hashKey)('foo')}>
        <span className={`${hashClassName(hashKey)('foo-bar')} ${hashClassName(hashKey)('bar-foo')}`}>
