...
function createTest(platform, env) {
  ...
  return cb => {
    gulp.src(tests, { read: false }).pipe(mocha()).pipe(sync(cb));
  };
}
  ...
  platforms.forEach(platform => {
    ...
    envs.forEach(env =>
      gulp.task(
        `test-${platform}-${env}`,
        [`build-${platform}-${env}`],
        createTest(platform, env),
      ),
    );
  });
  gulp.task('test-browser', cb =>
    runSequence('test-browser-dev', 'test-browser-prod', cb),
  );
  gulp.task('test-node', cb =>
    runSequence('test-node-dev', 'test-node-prod', cb),
  );
