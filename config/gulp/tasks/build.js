...
const sources = [path.join(lib, '**', '*.jsx'), path.join(lib, '**', '*.js')];

const misc = sources
  .map(source => `!${source}`)
  .concat(path.join(lib, '**', '*'));
...
function createCopy(platform, env) {
  return () =>
    gulp
      .src(misc)
      .pipe(
        changed(path.join(dist, platform, env, 'lib'), {
...
export default () => {
  gulp.task(
    'build',
    Object.keys(babelConfig).map(platform => {
      const buildPlatformTaskName = `build-${platform}`;
      gulp.task(
        buildPlatformTaskName,
        Object.keys(babelConfig[platform]).map(env => {
          const buildEnvTaskName = `build-${platform}-${env}`;
          const copyEnvTaskName = `copy-${platform}-${env}`;
          gulp.task(
            copyEnvTaskName,
            [`clean-${platform}-${env}`],
            createCopy(platform, env),
          );
          gulp.task(
            buildEnvTaskName,
            [copyEnvTaskName, 'lint'],
            createBuild(platform, env),
          );
